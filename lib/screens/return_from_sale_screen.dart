...
class _ReturnFromSaleScreenState extends State<ReturnFromSaleScreen> {
  ...
  ProductTransaction? selectedSale;
  int? selectedBankId;
  ...
  static const List<Color> _gradient = [Color(0xFF11998E), Color(0xFF38EF7D)];
  ...
  Future<void> _pickProduct() async {
    ...
    final soldProductIds = productProvider.productTransactions
        .where((t) => t.type == ProductTxType.sale && t.contactName == contactFullName)
        .map((t) => t.productId)
        .toSet();
    final availableProducts = productProvider.products.where((p) => soldProductIds.contains(p.id)).toList();
              ...
              content: SizedBox(
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(
                      child: filtered.isEmpty
                          ? Center(child: Text('محصولی به این مخاطب فروخته نشده', style: TextStyle(color: AppColors.textSecondary(dialogContext), fontWeight: FontWeight.w600, fontFamily: _fontFamily), textAlign: TextAlign.center))
                          : ListView.builder(
                              itemCount: filtered.length,
                              itemBuilder: (context, index) {
    ...
    if (result != null) {
      ...
      _pickSaleTransaction();
    }
  }

  Future<void> _pickSaleTransaction() async {
    ...
    final result = await showDialog<ProductTransaction>(
      ...
      builder: (dialogContext) => AlertDialog(
        backgroundColor: AppColors.card(dialogContext),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text('انتخاب فروش', style: TextStyle(fontWeight: FontWeight.w700, color: AppColors.text(dialogContext), fontFamily: _fontFamily)),
        content: SizedBox(
          width: double.maxFinite,
          height: 300,
          child: matches.isEmpty
              ? Center(child: Text('فروشی یافت نشد', style: TextStyle(color: AppColors.textSecondary(dialogContext), fontFamily: _fontFamily)))
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      trailing: Text('${formatAmount(t.totalAmount)} تومان', style: const TextStyle(fontWeight: FontWeight.w800, color: Color(0xFF11998E), fontFamily: _fontFamily)),
    ...
    if (result != null) setState(() => selectedSale = result);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final unitPrice = selectedSale?.pricePerUnit ?? 0;
        ...
        child: Column(
          ...
          children: [
            ...
            InkWell(
              ...
              child: Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(color: AppColors.card(context), borderRadius: BorderRadius.circular(14), border: Border.all(color: selectedContact == null ? AppColors.divider(context) : const Color(0xFF38EF7D), width: 2)),
                child: Row(
                  children: [
            ...
            if (selectedProduct != null)
              InkWell(
                onTap: _pickSaleTransaction,
                borderRadius: BorderRadius.circular(14),
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: AppColors.card(context), borderRadius: BorderRadius.circular(14), border: Border.all(color: selectedSale == null ? AppColors.divider(context) : const Color(0xFF38EF7D), width: 2)),
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          selectedSale == null ? 'انتخاب فروش (تاریخ) *' : '${_formatDateToJalali(selectedSale!.date)}  •  ${formatAmount(selectedSale!.pricePerUnit)} تومان',
                          style: TextStyle(color: selectedSale != null ? AppColors.text(context) : AppColors.textMuted(context), fontWeight: FontWeight.w600, fontFamily: _fontFamily),
                        ),
                      ),
                      Icon(Icons.arrow_forward_ios, size: 14, color: AppColors.textSecondary(context)),
                    ],
            ...
            if (selectedSale != null)
              Padding(
                ...
                child: Text('حداکثر: ${selectedSale!.quantity.toStringAsFixed(0)} عدد از این فروش', style: TextStyle(fontSize: 11, color: AppColors.textMuted(context), fontFamily: _fontFamily)),
              ),
            const SizedBox(height: 16),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('مبلغ پرداختی', style: TextStyle(color: AppColors.textSecondary(context), fontSize: 12, fontWeight: FontWeight.w600, fontFamily: _fontFamily)),
            ...
            Text('پرداخت مبلغ از (اختیاری)', style: TextStyle(color: AppColors.textSecondary(context), fontSize: 12, fontWeight: FontWeight.w600, fontFamily: _fontFamily)),
            const SizedBox(height: 8),
            ...
            if (selectedPaymentMethod == 'cash')
              InkWell(
                ...
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: AppColors.card(context), borderRadius: BorderRadius.circular(14), border: Border.all(color: selectedCashboxId == null ? AppColors.divider(context) : const Color(0xFF38EF7D), width: 2)),
                  child: Row(
                    children: [
            ...
            if (selectedPaymentMethod == 'card')
              InkWell(
                ...
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: AppColors.card(context), borderRadius: BorderRadius.circular(14), border: Border.all(color: selectedBankId == null ? AppColors.divider(context) : const Color(0xFF38EF7D), width: 2)),
                  child: Row(
                    children: [
  ...
  void _submit() async {
    if (selectedContact == null || selectedProduct == null || selectedSale == null || quantityController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('مخاطب، محصول، فروش و تعداد الزامی هستند', style: TextStyle(fontFamily: _fontFamily))));
      return;
    }
    ...
    if (quantity > selectedSale!.quantity) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تعداد برگشتی نمی‌تواند از تعداد فروخته‌شده بیشتر باشد', style: TextStyle(fontFamily: _fontFamily))));
      return;
    }
    ...
    final originalSaleTxId = selectedSale!.id!;
    ...
    await ledgerProvider.addEntry(LedgerEntry(
      ...
      sourceType: 'returnFromSale',
      productId: selectedProduct!.id,
      quantity: quantity,
      unitPrice: selectedSale!.pricePerUnit,
      unitCost: unitCost,
      relatedProductTxId: originalSaleTxId,
      logProductTxId: logTxId,
      affectedBankId: affectedBankId,
      bankAmount: selectedPaymentMethod != null ? totalReturn : null,
      bankIsIncome: selectedPaymentMethod != null ? false : null,
