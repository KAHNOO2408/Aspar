...
class AddDiscountScreen extends StatefulWidget {
  final DebtType type; // owed = تخفیف از خرید (بدهی خودت کم میشه) | receivable = تخفیف از فروش (طلبت کم میشه)
  const AddDiscountScreen({Key? key, required this.type}) : super(key: key);
...
class _AddDiscountScreenState extends State<AddDiscountScreen> {
  ...
  bool get isPurchaseDiscount => widget.type == DebtType.owed;
  List<Color> get gradient => isPurchaseDiscount ? const [Color(0xFFFF7A59), Color(0xFFE64A19)] : const [Color(0xFF11998E), Color(0xFF38EF7D)];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text(isPurchaseDiscount ? 'ثبت تخفیف از خرید' : 'ثبت تخفیف از فروش', style: const TextStyle(fontFamily: _fontFamily))),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      isPurchaseDiscount ? 'وقتی موقع تسویه‌ی بدهیت، فروشنده تخفیفی بهت میده' : 'وقتی موقع تسویه‌ی طلبت، به مشتری تخفیفی میدی',
                      style: const TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.w600, fontFamily: _fontFamily),
                    ),
                  ),
                ],
              ),
            ...
            TextField(
              ...
              decoration: _decoration(context, 'مبلغ تخفیف (تومان) *'),
            ),
            if (amount > 0)
              Padding(
                  ...
                  child: Padding(
                    ...
                    child: Center(
                      child: _isSubmitting
                          ...
                          : const Text('ثبت تخفیف', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 16, fontFamily: _fontFamily)),
                    ),
                  ),
                ),
              ),
  ...
  void _submit() async {
    if (selectedContact == null || amountController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('مخاطب و مبلغ الزامی هستند', style: TextStyle(fontFamily: _fontFamily))));
      return;
    }
    ...
    final description = noteController.text.isNotEmpty
        ? '${isPurchaseDiscount ? "تخفیف از خرید" : "تخفیف از فروش"} - ${noteController.text}'
        : (isPurchaseDiscount ? 'تخفیف از خرید' : 'تخفیف از فروش');

    // تخفیف از خرید: مثل یه پرداخت عمل می‌کنه (بدهیت کم میشه) ولی بدون جابه‌جایی پول واقعی
    // تخفیف از فروش: مثل یه دریافت عمل می‌کنه (طلبت کم میشه) ولی بدون جابه‌جایی پول واقعی
    await ledgerProvider.addEntry(LedgerEntry(
      personName: selectedContact!.firstName,
      personFamily: selectedContact!.lastName,
      date: selectedDate,
      description: description,
      debitAmount: isPurchaseDiscount ? amount : 0,
      creditAmount: isPurchaseDiscount ? 0 : amount,
      sourceType: 'discount',
    ));

    if (mounted) {
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تخفیف ثبت شد ✅', style: TextStyle(fontFamily: _fontFamily))));
    }
  }
